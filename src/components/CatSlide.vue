...
export default {
  ...
  methods: {
    initBox () {
      ...
      // 设定拉力系数
      this.maxPullinX = this.slideBoxWidth * .3
      this.maxPullinY = this.slideBoxHeight * .3
    },
    // touch
    touchStart (e) {
      // 记录开始的时间
    ...
    touchMove (e) {
      // 记录当前位置
      let {pageX: tempCurrentTouchX, pageY: tempCurrentTouchY} = e.changedTouches[0]
      // console.log('Current Touch: [x: %s, y: %s]', tempCurrentTouchX, tempCurrentTouchY)
      // 当前位置与开始位置的差值就是此次移动的距离
      let tempCurrentDifferX = tempCurrentTouchX - this.touchStartX
      let tempCurrentDifferY = tempCurrentTouchY - this.touchStartY
      // console.log('Differ Touch In: [x: %s, y: %s]', tempCurrentDifferX, tempCurrentDifferY)

      // 衔接位移 = 当前位移距离 + 上次最后位移距离
      let tempLastTouchX = tempCurrentDifferX + this.lastTouchX
      let tempLastTouchY = tempCurrentDifferY + this.lastTouchY
